...
called targets, which are called within `aggregate` and `estimate` contexts.
The target names should be familiar, and are all prefixed with `grm_`. Common
examples include:
...
and others. Users supply the desired variable to the target, whether or not to
annualize the estimate, and any other relevant arguments. Aside from the targets,
...
they are typically abstracted away when using GRM targets. Still, some inspection
...
## Estimation of Change Components with Targets
...
sufficient to use the GRM targets to specify the desired change components, which
...
mortality volume. The target `grm_mortality` is used, which generates mortality
...
Likewise, ingrowth can also be estimated with the `grm_ingrowth` target
...
Higher order change components are `grm_net_change`, `grm_net_growth` and
`grm_gross_growth`, all of which use convenient targets.
...
especially when normalizing to forested area. Estimates of this kind proceed
along the same lines as [ratio estimates](ratio_estimates.qmd). The only
modification, of course, is the specification of the GRM estimates themselves.
