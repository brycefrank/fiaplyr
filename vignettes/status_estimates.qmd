...
3. The average diameter of standing dead trees.

The first two use cases are post-stratified estimates, while the third is a
post-stratified ratio estimate. A standalone vignette is available for ratio
estimates [here, todo].
...
## Average Diameter of Standing Dead Trees

```{r}
standing_dead_handler <- handler |>
  filter_tree(DIA >= 5, STATUSCD == 2) |>
  set_tree_domains(SPGRPCD)

psr_estimator <- PostStratifiedRatioEstimator(standing_dead_handler)

psr_estimator |>
  estimate_ratio(tree ~ DIA, tree ~ 1) |>
  filter(SPGRPCD_n == SPGRPCD_d)
```